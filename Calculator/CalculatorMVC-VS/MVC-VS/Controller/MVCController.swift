...
class MVCController: UIViewController {
    ...
    private func setupEvents() {
        ...
        self.observers += [NotificationCenter.default.addObserver(forName: UITextField.textDidChangeNotification, object: nil, queue: nil, using: { (note) in
            self.viewState?.textFieldValue = (note.object as! UITextField).text ?? ""
        })];
        ...
        self.calculator.performAction = { [weak self] (textField) in
            if let self = self {
                let formula = self.viewState?.textFieldValue ?? "0";
    ...
    private func setupStates() {
        self.viewState = MVCViewState.init(textFieldValue: model.result.value)
    }
